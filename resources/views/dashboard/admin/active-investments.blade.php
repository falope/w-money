                    ...
                    <div class="card-header border-0">
                        <div class="row align-items-center">
                            ...
                            @if(isset($_GET['search_investment']))
                                    ...
                                    @if($planQ != 'all')
                                        <a href="{{ route('download-active-investment-csv', ['type' => $planQ ]) }}"
                                    class="float-right btn btn-sm btn-success text-white"> Download {{ $planQ }} Records CSV </a>
                                    @else
                                        <a href="{{ route('download-active-investment-csv', ['type' => 'all' ]) }}"
                                ...
                                @if (count($investments) > 0)
                                    @foreach ($investments as $count => $investment)
                                        <tr>
                                            ...
                                            <td> <?php echo date('F j, Y', strtotime(date('Y-m-d',
                                                strtotime('+' . $investment->duration . ' ' . $investment->duration_type .
                                                's', strtotime($investment->activated_on))))); ?></td>
                                            <td> Active </td>
                                            <td>
                                                <div>
                                                    <div class="form-group" style="margin-bottom: 5px">
                                                        <input type="number" placeholder="Withdraw amount"
                                                            class="form-control-sm withdraw_amount">

                                                    </div>
                                                    <button class='btn btn-danger btn-sm withdraw-investment'
...
@section('script')
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('.withdraw-investment').on('click', function() {

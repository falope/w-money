...
@section('title', 'Edit Plan | Wmoney.ng ')
            ...
            <div class="col-md-12 mb-5 mb-xl-0">
                <div class="card shadow" style="padding:20px">
                    ...
                    <div style="margin: 0; margin-bottom: 20px; margin-top: 20px">
                        ...
                        <form id="add-plan-form" method="post" action="{{ route('update-plan') }}"
                            enctype="multipart/form-data">
                            <div class="form-group">
                                <label for="exampleFormControlSelect1">Plan Name</label>
                                <input name="name" required type="text" class="form-control" value="{{ $plan->name }}"
                                    placeholder="Plan Name">
                            </div>
                            <input type="hidden" name="slug" value="{{ $plan->slug }}">
                            <div class="form-group">
                                <label for="exampleFormControlSelect1">Plan Amount</label>
                                <input name="amount" required type="number" value="{{ $plan->amount }}"
                                    class="form-control" placeholder="Plan Amount">
                            </div>
                            <div class="form-group">
                                <label for="exampleFormControlSelect1">Plan Duration</label>
                                <input name="duration" required type="number" value="{{ $plan->duration }}"
                                    class="form-control" placeholder="Plan Duration">
                            </div>
                            <div class="form-group">
                                <label for="exampleFormControlSelect1">Plan Duration Type</label>
                                <select name="duration_type" class="form-control">
                                    <option>Select duration type</option>
                                    <option <?php echo $plan->duration_type === 'year' ? 'selected' : ''; ?> value="year">Year</option>
                                    <option <?php echo $plan->duration_type === 'month' ? 'selected' : '';
                                        ?> value="month">Month</option>
                                    <option <?php echo $plan->duration_type === 'week' ? 'selected' : ''; ?> value="week">Week</option>
                                    <option <?php echo $plan->duration_type === 'day' ? 'selected' : ''; ?> value="day">Day</option>
                                </select>
                            </div>
                            <input type="hidden" name="meta">
                            <div class="form-group">
                                <label>Image URL</label>
                                <input type="text" name="imageUrl" class="form-control" placeholder="Paste Cloudinary image link" value="{{ $plan->image }}">
                            </div>
                            <style>
                                .plan-range {
                            ...
                            <div id="plan-meta" style="margin-bottom: 20px">
                                ...
                                @foreach ((object) json_decode($plan->meta) as $count => $meta)
                                    <?php [$a, $b] = $meta->range; ?>
                                    <div class="plan-range"
                                        style="margin-top: 20px; border: 2px solid rgba(0,0,0,0.1); padding: 10px; border-radius:5px">
                                        <div class="form-group">
                                            <label> Range :</label>
                                            <input class="form-control" type="number" name="range[{{ $count }}][0]"
                                                value="{{ $a }}">
                                            <input class="form-control" type="number" name="range[{{ $count }}][1]"
                                                value="{{ $b }}">
                                            <small> If it is
                                        ...
                                        <div class="form-group">
                                            <label> ROI :</label>
                                            <input type="number" class="form-control" name="range[{{ $count }}][2]"
                                                value="{{ $meta->roi }}">
                                        </div>
                                    </div>
                                @endforeach
                            </div>
                            <button class="btn btn-success btn-block add-plan" type="button">Update plan</button>
                        </form>
                    </div>
                </div>
            </div>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('.add-range').on('click', function() {
                ...
                i = lastRange[0] + 1;
                var html =
                    ` <div class="plan-range" style="margin-top: 20px; border: 2px solid rgba(0,0,0,0.1); padding: 10px; border-radius:5px">
                                                                                                                                        <div class="form-group">
                                                                                                                                            <label> Range :</label>
